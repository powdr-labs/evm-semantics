...
/-- Pretty-print an `Option UInt256` either as a 64-char hex string
    or the literal `<none — RLP overflow>` (which a gas-bounded EVM
    execution can't reach). -/
def hexOpt (v : Option UInt256) : String :=
  match v with
  | some h => hex64 h
  | none   => "<none — RLP overflow>"
...
def go : IO Unit := do
  ...
  IO.println s!"empty trie root: {hexOpt emptyRoot}"
  IO.println s!"  expected      : {emptyTrieRootExpected}"
  IO.println s!"  ok            : {hexOpt emptyRoot == emptyTrieRootExpected}"
  ...
  IO.println s!"storage[0]=1 root: {hexOpt r}"
