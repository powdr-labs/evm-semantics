...
```
pass(root=9657 full+=0 core+=7) fail=0 incon=0 crash=0
```
...
- `pass_root` = world MPT `stateRoot` matches the corpus's
  ...
  Geth would produce). 9657 of 9664.
- `pass_full` = every field the test's `postState` enumerates matches
  (storage, nonce, code, *and* balance), but the MPT root differs.
  Currently 0.
- `pass_core` = storage / nonce / code match but balance is off. 7 of
  9664: six `randomStatetest*` / `Call1024PreCalls_Constantinople`
  variants with ±4800/9600-wei drifts (Constantinople-era SSTORE
  refund accounting corner — EIP-1283 net-metered refunds), plus one
  `tx_e1c174e2_EIP150` with a large one-off diff.
