#!/usr/bin/env bash
#
# Normalize raw `blockchaintests -v <dir>` output into a stable, diffable summary of
...
# (`.github/blockchaintests-expected-failures.txt`): merges stay conflict-free
...
#
#   .github/scripts/blockchaintests_summary.sh <raw> > .github/blockchaintests-expected-failures.txt
#
...
# BlockchainTests runner (`blockchaintests`) prints the same aggregate line and
# the same `FAIL <id>: …` / `INCON <id>: …` / `CRASH <id>: …` per-test notes.
#
# Usage: blockchaintests_summary.sh <raw-output-file>
set -euo pipefail

raw="${1:?usage: blockchaintests_summary.sh <raw-output-file>}"

# Sanity: refuse to emit an empty summary for an incomplete run.
if ! grep -qE 'pass\(root=[0-9]+ full\+=[0-9]+ core\+=[0-9]+\) fail=[0-9]+' "$raw"; then
  echo "blockchaintests_summary.sh: no aggregate 'pass(root=… full+=… core+=…) fail=…' line" \
