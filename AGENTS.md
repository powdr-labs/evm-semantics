...
- **`Step : State → State → Prop`** (`EVM/Step.lean`) — small-step relation,
  ...
  premises vary — `Step.stop` (and the other halts) has no `h_gas`/`h_stack`,
  and stackless reads (`address`, `coinbase`, `pc`, …) have no `h_stack`. Read
  the actual constructor; `consumeGas` takes the gas-sufficiency proof
