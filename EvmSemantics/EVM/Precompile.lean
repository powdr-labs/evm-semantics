...
----------------------------------------------------------------------------
-- EIP-2537 MSM gas discount tables (indexed by pair count `k`, 1-based).
----------------------------------------------------------------------------

/-- G1MSM per-`k` discount (÷1000), `k = 1..128`; `k > 128` uses the last
    (minimum) entry `519`. EIP-2537. -/
def blsG1MsmDiscount : Array Nat :=
...
/-- G2MSM per-`k` discount (÷1000), `k = 1..128`; `k > 128` uses the last
    (minimum) entry `524`. EIP-2537. -/
def blsG2MsmDiscount : Array Nat :=
  #[1000, 1000, 923, 884, 855, 832, 812, 796, 782, 770, 759, 749, 740, 732, 724, 717,
    ...
    546, 545, 544, 543, 542, 541, 541, 540, 539, 538, 537, 537, 536, 535, 535, 534,
    533, 532, 532, 531, 530, 530, 529, 528, 528, 527, 526, 526, 525, 524, 524]

/-- EIP-2537 MSM gas: `⌊k · base · discount(k) / 1000⌋`, with `discount(k)`
    read from `table` (clamped to the last entry for `k > 128`). `k = 0`
    (empty input) is rejected by the driver, so it costs `0` here. -/
@[inline] def blsMsmGas (base : Nat) (table : Array Nat) (k : Nat) : Nat :=
  if k == 0 then 0
  else
    let discount := (table[Nat.min k 128 - 1]?).getD (table[table.size - 1]!)
    k * base * discount / 1000
...
/-- Gas: EIP-2537 discounted `⌊k · 12000 · discount(k) / 1000⌋` with
    `k = |input| / 160`. Invalid empty input (`k = 0`) is rejected by the
    driver. -/
@[inline] def blsG1MsmGas (input : ByteArray) : Nat :=
  blsMsmGas 12000 blsG1MsmDiscount (input.size / 160)
...
/-- Gas: EIP-2537 discounted `⌊k · 22500 · discount(k) / 1000⌋` with
    `k = |input| / 288`. -/
@[inline] def blsG2MsmGas (input : ByteArray) : Nat :=
  blsMsmGas 22500 blsG2MsmDiscount (input.size / 288)
